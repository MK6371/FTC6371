...
@Autonomous(name="vision autonomous red", group="vision")
public class VisionAutonomousRed extends LinearVisionOpMode {
    ...
    private void moveToNext() throws InterruptedException{
        ...
        controller.setTuning(0.0025, 0, 0, 0);
    ...
    private void turnToVortex(boolean farSide) throws InterruptedException {
        robot.move(0.25, 0.25, 0.25, 0.25);
        if(blueAlliance) {
            while (Math.abs(robot.frontLeft.getCurrentPosition()) < 250 && opModeIsActive()) {
            }
        } else {
            if(farSide) {
                while (Math.abs(robot.frontLeft.getCurrentPosition()) < 2000 && opModeIsActive()) {
